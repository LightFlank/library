...
class KBCOptimizer(object):
    def __init__(
            ...
            verbose: bool = True,hr2t=None,tr2h=None,t2hr=None,h2tr=None,hr2rt=None,tr2rh=None,t2rrh=None,h2rrt=None,we=0,wer=0
    ):
        self.model = model
    ...
    def epoch(self, examples: torch.LongTensor, e=0, weight=None):
        ...
        for i in actual_examples:
                ...
                if h_neibor_dict:
                    ...
                    if i[0].item()==random_h:
                        weight = 1.0
                    else:
                        a = self.h2rrt[i[0].item()]
                        b = self.h2rrt[random_h]
                        intersection = a & b
                ...
                if t_neibor_dict:
                    ...
                    if i[2].item()==random_t:
                        weight = 1.0
                    else:
                        a = self.t2rrh[i[2].item()]
                        b = self.t2rrh[random_t]
                        intersection = a & b
                        union = len(a) + len(b) - len(intersection)
                        weight = len(intersection) / union if union > 0 else 0
                    p_t.append(random_t)
                ...
                if hr_neibor_dict:
                    ...
                    if (i[0].item(), i[1].item())==random_hr:
                        ...
                        a=self.hr2rt[(i[0].item(), i[1].item())]
                        b=self.hr2rt[random_hr]
                        intersection =a & b
                        union=len(a)+len(b)-len(intersection)
                        weight=len(intersection)/union if union > 0 else 0
                ...
                if tr_neibor_dict:
                    ...
                    if (i[2].item(), i[1].item())==random_tr:
                        ...
                        a=self.tr2rh[(i[2].item(), i[1].item())]
                        b=self.tr2rh[random_tr]
                        intersection =a & b
                        union=len(a)+len(b)-len(intersection)
                        weight=len(intersection)/union if union > 0 else 0
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while b_begin < examples.shape[0]:
                ...
                l = l_fit + l_reg+(l3+l4)*self.we+(l5+l6)*self.wer
                ...
                bar.set_postfix(loss=f'{l.item():.1f}', reg=f'{l_reg.item():.1f}',l_h=f'{l3.item():.1f}',l_t=f'{l4.item():.1f}',l_hr=f'{l5.item():.1f}',l_tr=f'{l6.item():.1f}')
